...
async function fetchCN() {
  sys.log.info("[Fetch] fetching cn prompts...");
  try {
    ...
    sys.log.error("[Fetch] failed to fetch cn prompts", error);
    return [];
  }
}

async function fetchTW() {
  sys.log.info("[Fetch] fetching tw prompts...");
  try {
    ...
    sys.log.error("[Fetch] failed to fetch tw prompts", error);
    return [];
  }
}

async function fetchEN() {
  sys.log.info("[Fetch] fetching en prompts...");
  try {
    ...
    sys.log.error("[Fetch] failed to fetch en prompts", error);
    return [];
  }
}
...
async function main() {
  Promise.all([fetchCN(), fetchTW(), fetchEN()])
    ...
    .catch((e) => {
      sys.log.error("[Fetch] failed to fetch prompts");
      fs.writeFile(FILE, JSON.stringify({ cn: [], tw: [], en: [] }));
    })
    .finally(() => {
      sys.log.info("[Fetch] saved to " + FILE);
    });
}
