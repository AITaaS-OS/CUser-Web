  ...
  return {
    async check() {
      try {
        ...
        sys.log.info("[Upstash] check", res.status, res.statusText);
        return [200].includes(res.status);
      } catch (e) {
        sys.log.error("[Upstash] failed to check", e);
      }
      return false;
    },
    ...
    async redisGet(key: string) {
      ...
      sys.log.info("[Upstash] get key = ", key, res.status, res.statusText);
    ...
    async redisSet(key: string, value: string) {
      ...
      sys.log.info("[Upstash] set key = ", key, res.status, res.statusText);
    },
    ...
    async get() {
      ...
      sys.log.info("[Upstash] get full chunks", chunks);
      return chunks.join("");
    },
