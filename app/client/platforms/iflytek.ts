...
export class SparkApi implements LLMApi {
  ...
  path(path: string): string {
    ...
    if (accessStore.enableCustomConfig) {
      baseUrl = accessStore.iflytekBaseUrl;
    }

    if (baseUrl.length === 0) {
      const isApp = !!getClientConfig()?.isApp;
      const apiPath = ServerAPIPath.Iflytek;
      baseUrl = isApp ? Iflytek.BaseUrl : apiPath;
    }

    if (baseUrl.endsWith("/")) {
      baseUrl = baseUrl.slice(0, baseUrl.length - 1);
    }
    if (!baseUrl.startsWith("http") && !baseUrl.startsWith(ServerAPIPath.Iflytek)) {
  ...
  async chat(options: ChatOptions) {
    ...
    sys.log.info("[Request] Spark payload: ", requestPayload);
      ...
      if (shouldStream) {
        ...
        fetchEventSource(chatPath, {
          ...
          async onopen(res) {
            ...
            sys.log.info("[Spark] request response content type: ", contentType);
            responseRes = res;
            ...
            if (
              ...
              options.onError?.(
                new Error(
                  `Request failed with status ${res.status}: ${responseText}`,
                ),
              );
          ...
          onmessage(msg) {
            ...
            try {
              ...
              sys.log.error("[Request] parse error", text);
