...
export class DoubaoApi implements LLMApi {
  path(path: string): string {
    ...
    if (accessStore.enableCustomConfig) {
      baseUrl = accessStore.bytedanceBaseUrl;
    }

    if (baseUrl.length === 0) {
      const isApp = !!getClientConfig()?.isApp;
      baseUrl = isApp ? ByteDance.BaseUrl : ServerAPIPath.ByteDance;
    }

    if (baseUrl.endsWith("/")) {
      baseUrl = baseUrl.slice(0, baseUrl.length - 1);
    }
    if (!baseUrl.startsWith("http") && !baseUrl.startsWith(ServerAPIPath.ByteDance)) {
