import { ServerAPIPath, Google, REQUEST_TIMEOUT_MS } from "@/app/constant";
import {
  ChatOptions,
  getHeaders,
  LLMApi,
...
export class GeminiProApi implements LLMApi {
  path(path: string, shouldStream = false): string {
    ...
    if (accessStore.enableCustomConfig) {
      baseUrl = accessStore.googleBaseUrl;
    }

    const isApp = !!getClientConfig()?.isApp;
    if (baseUrl.length === 0) {
      baseUrl = isApp ? Google.BaseUrl : ServerAPIPath.Google;
    }
    if (baseUrl.endsWith("/")) {
      baseUrl = baseUrl.slice(0, baseUrl.length - 1);
    }
    if (!baseUrl.startsWith("http") && !baseUrl.startsWith(ServerAPIPath.Google)) {
  ...
  extractMessage(res: any) {
    sys.log.info("[Response] gemini-pro response: ", res);
  ...
  async chat(options: ChatOptions): Promise<void> {
    ...
    const messages = _messages.map((v) => {
      ...
      if (isVisionModel(options.config.modelName)) {
        const images = getMessageFiles(v);
        if (images.length > 0) {
          multimodal = true;
          parts = parts.concat(
            images.map((image) => {
              const imageType = image.url.split(":")[1];
