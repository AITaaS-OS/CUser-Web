import { Anthropic, ServerAPIPath } from "@/app/constant";
...
import { ChatMessageTool } from "@/app/types";
...
export class ClaudeApi implements LLMApi {
  ...
  extractMessage(res: any) {
    sys.log.info("[Response] claude response: ", res);
  ...
  async chat(options: ChatOptions): Promise<void> {
    ...
    for (const v of options.messages) {
      const content = await file.preProcessImageContent(v.content);
    ...
    const prompt = messages
      ...
      .map((v) => {
        ...
        return {
          role: insideRole,
          content: content
            .filter((v) => v.url || v.text)
            .map(({ type, text, url: urlVal }) => {
              if (type === "text") {
                return {
                  type,
                  text: text!,
              ...
              // urlVal can be either a string data URL or an object containing a `url` property
              let dataUrl = "";
              if (typeof urlVal === "string") {
                dataUrl = urlVal;
              } else if (urlVal && typeof urlVal === "object") {
                dataUrl = (urlVal as any).url || "";
              }
              ...
              const mimeType = dataUrl.slice(colonIndex + 1, semicolonIndex);
              const encodeType = dataUrl.slice(semicolonIndex + 1, comma);
    ...
    if (shouldStream) {
      ...
      try {
        ...
        sys.log.error("failed to chat", e);
  ...
  path(path: string): string {
    ...
    if (accessStore.enableCustomConfig) {
      baseUrl = accessStore.anthropicBaseUrl;
    }
    ...
    if (baseUrl.trim().length === 0) {
      ...
      baseUrl = isApp ? Anthropic.BaseUrl : ServerAPIPath.Anthropic;
    }
