...
export class ErnieApi implements LLMApi {
  path(path: string): string {
    ...
    if (accessStore.enableCustomConfig) {
      baseUrl = accessStore.baiduBaseUrl;
    }

    if (baseUrl.length === 0) {
      const isApp = !!getClientConfig()?.isApp;
      // do not use proxy for baidubce api
      baseUrl = isApp ? Baidu.BaseUrl : ServerAPIPath.Baidu;
    }

    if (baseUrl.endsWith("/")) {
      baseUrl = baseUrl.slice(0, baseUrl.length - 1);
    }
    if (!baseUrl.startsWith("http") && !baseUrl.startsWith(ServerAPIPath.Baidu)) {
  ...
  async chat(options: ChatOptions) {
    ...
    sys.log.info("[Request] Baidu payload: ", requestPayload);
      ...
      if (shouldStream) {
        ...
        fetchEventSource(chatPath, {
          ...
          async onopen(res) {
            ...
            sys.log.info("[Baidu] request response content type: ", contentType);
            responseRes = res;
